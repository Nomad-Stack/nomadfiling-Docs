...
<ul>
  <li>Checks that the token starts with `nf_`</li>
  <li>Computes the SHA-256 hash of the raw token</li>
  <li>Compares that hash to `api_keys.key_hash`</li>
  <li>Verifies the row exists in `api_keys`</li>
  <li>Verifies `expires_at` is either `null` or in the future</li>
  <li>Updates `api_keys.last_used_at` asynchronously</li>
  <li>Returns `user_id` on success, or `null` on failure</li>
</ul>
...
<ul>
  <li>Checks that the token starts with `mcp_at_`</li>
  <li>Computes the SHA-256 hash of the raw token</li>
  <li>Compares that hash to `oauth_tokens.access_token_hash`</li>
  <li>Verifies the row exists in `oauth_tokens`</li>
  <li>Verifies `revoked` is `false`</li>
  <li>Verifies `expires_at` is in the future</li>
  <li>Updates `oauth_tokens.last_used_at` asynchronously</li>
  <li>Returns `user_id` on success, or `null` on failure</li>
</ul>
